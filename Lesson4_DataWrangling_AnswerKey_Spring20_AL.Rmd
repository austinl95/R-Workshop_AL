...
anxiety %>% select(Subject_ID, Age, SES, -Drug_Use) %>% summary()


#You can also nest a filter function within a select function
...
```{r, comment=NA}
#Add variables from existing ones. Let's pretend that the chance of developing major depressive disorder is your two times your drug_use index. And your probability of seeking therapy is 3 times the stress index minus their self reported anxiety level.
...
anxiety %>%
  select(Age, Treatment_Group, SES, Anxiety_SelfReport, Depression) %>%
  group_by(Treatment_Group) %>%
  summarize(mean_age = mean(Age),
            mean_SES = mean(SES),
            mean_SelfReport = mean(Anxiety_SelfReport),
            mean_Depression = mean(Depression))

#Grouping by three age groups
anxiety %>%
  select(Age, Treatment_Group, SES, Anxiety_SelfReport, Depression, agegroup) %>%
  group_by(agegroup) %>%
  summarize(mean_age = mean(Age),
            mean_SES = mean(SES),
            mean_SelfReport = mean(Anxiety_SelfReport),
