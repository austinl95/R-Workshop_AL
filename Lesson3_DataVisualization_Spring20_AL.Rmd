---
title: "Lesson3_DataVisualization_Spring20_AL"
author: "Austin Luor"
date: "5/2/2020"
...
ggplot(anxiety, aes(Treatment_Group, Stress, fill=Trea))
...
#Error bar
boxplot2 + scale_x_discrete(limit=c("YA","MA","OA")) + ylim(0,50) +  stat_summary(fun.y = mean, fun.ymin = min, fun.ymax = max, geom="errorbar", width=0.05)
...
longformat+ scale_fill_brewer(palette="YlOrRd")
...
longformat + scale_fill_manual(values=c("green", "blue", "pink"))
```
...
and whatever customization you want to add! 
```{r}

ggplot(data= Japan, mapping=       ) +       + 
...
```{r}
a <- ggplot() + geom_col(position = "dodge") + xlab("Chinese Cities") + labs(title = "China") + scale_fill_brewer(palette = "YlOrRd")

b <- ggplot(Japan, aes() 

c <- ggplot(South_Korea, aes(x=type, y=cases, fill=type)) 

d <-ggplot(US, aes(x=type, y=cases, fill=type)) +       +      + scale_fill_manual(values=c("#CCFF00", "#FF00FF", "#0099FF"))

e <- 

#Patch it up! If you are unsure of patchwork package's syntax, use the help function or google! 
...
#Gotta catch em all
ggplot(d, aes(x, y)) + geom_pokemon(aes(image=ifelse(x>0, 'pichu', 'charmander')), size=.1) + theme_classic()
